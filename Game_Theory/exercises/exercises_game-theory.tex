...
\begin{istgame}[sloped, above right,font=\tiny]
	...
	\istb{(0,1,3)}[above,near end]
	\istb{(0,0,3)}[above,near end]
	\istb{(0,2,2)}[above,near end]
	\istb{(0,2,1)}[above,near end]
	\istb{(0,2,0)}[above,near end]
	\endist
\end{istgame}
...
% 3.6 In a single-player game in which at each vertex $x$ that is not the root the player has $m_x$ actions, how many strategies has the player got?
...
% Player I has four strategies, $T_{1}T_{2}$, $T_{1}B_{2}$, $B_{1}T_{2}$, $B_{1}B_{2}$. Two of these strategies, $B_{1}T_{2}$ and $B_{1}B_{2}$, regardless of the strategy used by Player II, yield the same play of the game, because if Player I has selected action $B_{1}$ at the root vertex, he will never get to his second decision vertex. We can therefore eliminate one of these two strategies and define a _reduced strategy_$B_{1}$, which only stipulates that Player I chooses $B_{1}$ at the root of the game. In the game appearing in the above figure, the reduced strategies of Player I are $T_{1}T_{2}$, $T_{1}B_{2}$, and $B_{1}$. The reduced strategies of Player II are the same as his regular strategies, $t_{1}t_{2}$, $t_{1}b_{2}$, $b_{1}t_{2}$, and $b_{1}b_{2}$, because Player II does not know to which vertex Player I's choice will lead. Formally, a _reduced strategy_$\tau_{i}$ of player $i$ is a function from a subcollection $\widehat{\mathcal{U}_{i}}$ of player $i$'s collection of information sets to actions, satisfying the following two conditions:
...
% 		Footnote 8: A _continuous path_ is a chain of adjacent hexagons, where two hexagons are called “adjacent” if they share a common edge.
...
% 		Footnote 10: Equivalently, “x or y” = x ∨ y = max{x, y}, and “x and y” = x ∧ y = min{x, y}.
